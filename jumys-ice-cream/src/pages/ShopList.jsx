...
import ShopProduct from '../Components/ShopProduct';
import ScrollToTopButton from "../Components/TopButton";
// import './ShopList.css'; // Assume we use this CSS file for styles
...
const ShopList = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setIsLoading(false); // Hide the loader and display the content
      setIsPageLoaded(true); // Trigger the staggered animation for the content
    }, 2000); // 2-second delay for the animation
  ...
  return (
    <div>
      {isLoading ? (
        ...
        <div className={`content-container ${isPageLoaded ? 'staggered-fade-zoom' : ''}`}>
          <ScrollToTopButton />
          <div className="relative w-full h-[400px] page-header">
            <div className="absolute inset-0 flex items-center justify-center text-animation">
              <div className="flex flex-col text-center items-center justify-center text-white mt-28">
                <h1 className="text-4xl font-bold text-black">Shop List</h1>
                <p className="mt-2">
                  <a href="#" className="text-[#000] hover:text-[#ff0000]">Home</a>
          ...
          <div className="flex flex-col md:flex-row p-4 staggered-content">
            <div className="md:hidden mb-4 staggered-element">
              <Button variant="outlined" onClick={toggleFilter}>
                {showFilter ? 'Hide Filters' : 'Show Filters'}
              </Button>
            </div>

            <div className={`md:w-1/3 ${showFilter ? 'block' : 'hidden'} md:block mb-4 mt-12 md:mb-0 staggered-element`}>
              <FilterComponent
                onColorSelect={setSelectedColor}
                onPriceChange={handlePriceChange}
                onSizeSelect={setSelectedSize}
            ...
            <div className="md:w-2/3 staggered-element">
              <ShopProduct
                selectedColor={selectedColor}
                minPrice={minPrice}
                maxPrice={maxPrice}
